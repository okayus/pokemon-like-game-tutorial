 ...
 * 初学者向け：ダメージ計算で使用する数値
 */
export type タイプ相性倍率 = 0 | 0.25 | 0.5 | 1 | 2 | 4;
...
export function getBattleAdvice(
  ...
  const bestMoves = moveEffectiveness.filter((m) => m.result.is_critical).map((m) => m.type);
  ...
  if (bestMoves.length > 0) {
    ...
    const neutralMoves = availableMoveTypes.filter((type) => !worstMoves.includes(type));
