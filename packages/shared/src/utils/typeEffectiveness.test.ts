...
describe('タイプ相性システム', () => {
  ...
  describe('説明文とアドバイス', () => {
    it('タイプ相性の説明文を正しく生成する', () => {
      expect(getTypeEffectivenessExplanation('でんき', 'みず'))
        .toBe('でんき技はみずタイプによく効く（ダメージ2倍）');
      
      expect(getTypeEffectivenessExplanation('でんき', 'でんき'))
        .toBe('でんき技はでんきタイプにあまり効かない（ダメージ半減）');
      
      expect(getTypeEffectivenessExplanation('でんき', 'じめん'))
        .toBe('でんき技はじめんタイプに全く効かない');
      
      expect(getTypeEffectivenessExplanation('ノーマル', 'でんき'))
        .toBe('ノーマル技はでんきタイプに普通の効果');
    });
  ...
  describe('エッジケース', () => {
    ...
    it('すべてのタイプの組み合わせで数値が定義されている', () => {
      const types = ['ノーマル', 'でんき', 'みず', 'ひこう', 'くさ', 'ほのお', 'じめん', 'いわ', 'かくとう', 'エスパー'] as const;
