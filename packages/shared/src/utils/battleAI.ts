...
export class BattleAI {
   ...
   * 初学者向け：各技の有効性を数値化
   */
  private evaluateMoves(myPokemon: 参戦ポケモン, enemyPokemon: 参戦ポケモン): 技評価[] {
    return myPokemon.moves.map((move) => {
      // ダメージスコア（技威力）
      const damageScore = Math.min(100, (move.power / 120) * 100);

      // タイプ相性スコア
      const typeResult = calculateTypeEffectiveness(
        move.type,
        enemyPokemon.species_id.toString() as unknown as 技タイプ
      );
  ...
  private intermediateAction(
    ...
    const usableEvaluations = moveEvaluations.filter(
      (evaluation) => evaluation.move.current_pp > 0
    );
    ...
    if (
      ...
      situation.my_hp_percentage < 0.15 &&
      Math.random() < 0.4
    ) {
      return {
        action_type: '逃走',
  ...
  private advancedAction(
    ...
    if (situation.should_be_aggressive) {
      ...
      selectedEvaluation = usableEvaluations.reduce((best, current) =>
        ...
        best.damage_score + best.type_effectiveness_score
          ? current
          : best
      );
