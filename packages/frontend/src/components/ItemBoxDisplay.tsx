...
function ItemBox({
  ...
  // プレイヤーが近くにいる場合はハイライト
  const ハイライトクラス =
    isNearPlayer && box.状態 === '未開封' ? 'ring-2 ring-yellow-400 animate-pulse' : '';
...
export function ItemBoxDisplay({
  ...
  const openItemBox = async (box: アイテムボックス) => {
    ...
    try {
      ...
      if (result.success) {
        setローカルボックス一覧((prev) =>
          prev.map((b) => (b.id === box.id ? { ...b, 状態: '開封済み' as const } : b))
        );
      }
  ...
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      if (e.code === 'Space') {
        ...
        const nearbyBox = ローカルボックス一覧.find(
          (box) => box.状態 === '未開封' && isPlayerNearBox(box)
        );
    ...
    <div className="absolute inset-0 pointer-events-none">
      {ローカルボックス一覧.map((box) => (
        <div key={box.id} className="pointer-events-auto">
          <ItemBox box={box} isNearPlayer={isPlayerNearBox(box)} onOpen={() => openItemBox(box)} />
        </div>
      ))}
    </div>
  );
