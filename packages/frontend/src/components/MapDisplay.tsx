 ...
 */
export default function MapDisplay({ マップ, プレイヤー位置, onタイルクリック }: MapDisplayProps) {
  ...
  useEffect(() => {
    ...
    for (let y = 0; y < マップ.高さ; y++) {
      for (let x = 0; x < マップ.幅; x++) {
        ...
        // 出口の描画（特別なマーカー）
        const 出口 = マップ.出口.find((e) => e.位置.x === x && e.位置.y === y);
        if (出口) {
          描画出口マーカー(ctx, x, y);
        }
      }
   ...
   */
  function 描画タイル(ctx: CanvasRenderingContext2D, x: number, y: number, タイル: タイル) {
    // タイルの色を設定
    ctx.fillStyle = タイル色設定[タイル.タイプ] || '#000000';

    // タイルを塗りつぶし
    ctx.fillRect(x * タイルサイズ, y * タイルサイズ, タイルサイズ, タイルサイズ);

    // タイルの境界線を描画
    ctx.strokeStyle = '#00000020';
    ctx.strokeRect(x * タイルサイズ, y * タイルサイズ, タイルサイズ, タイルサイズ);
    ...
    if (タイル.草むら) {
      ...
      // 草の模様を描画
      for (let i = 0; i < 3; i++) {
        ctx.fillRect(x * タイルサイズ + 8 + i * 8, y * タイルサイズ + 20, 2, 8);
      }
    }
  }
   ...
   */
  function 描画出口マーカー(ctx: CanvasRenderingContext2D, x: number, y: number) {
    ctx.save();
   ...
   */
  function 描画プレイヤー(ctx: CanvasRenderingContext2D, x: number, y: number) {
    ctx.save();
  ...
  function handleCanvasClick(event: React.MouseEvent<HTMLCanvasElement>) {
    ...
    // 範囲内かチェック
    if (タイルX >= 0 && タイルX < マップ.幅 && タイルY >= 0 && タイルY < マップ.高さ) {
      onタイルクリック(タイルX, タイルY);
    }
  }
