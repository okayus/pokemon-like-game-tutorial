  ...
  async パーティ編成(player_id: string, 編成リクエスト: パーティ編成リクエスト): Promise<void> {
    if (編成リクエスト.pokemon_id) {
      ...
      `
        ...
        .bind(player_id, 編成リクエスト.position, 編成リクエスト.pokemon_id)
        .run();
    } else {
      // 指定位置からポケモンを削除
      await this.db
      ...
      `
        ...
        .bind(player_id, 編成リクエスト.position)
        .run();
    }
  }
